...
class Pipeline:
        ...
        try:
            ...
            if create_collage:
                ...
                if add_labels:
                    ...
                    if not state.is_complete("collage_margins"):
                        print(f"  Adicionando margens...")
                        t_marg = time.perf_counter()
                        FrameProcessor.batch_add_margins(frame_paths, 30)
                        timings["collage_margins"] = round(time.perf_counter() - t_marg, 1)
                        print(f"    margens: {timings['collage_margins']}s")
                        state.mark_complete("collage_margins")
                    else:
                        timings["collage_margins"] = 0
            ...
            if add_labels:
                if not state.is_complete("frame_headers"):
                    ...
                    FrameProcessor.batch_add_frame_headers(frame_paths, title_line1, label_map, line1_size=36, line2_size=160, padding_top=280, gap=10, subtitle=title_line2, subtitle_size=22)
    ...
    def _describe_processor(self, processor_name: Optional[str]) -> Optional[Dict[str, Any]]:
        ...
        descs = {
            ...
            "decode_patch_size_col101": {"processing": "unmask(0) para fundo cinza, valores 0-7 (tamanho do fragmento)", "divide_by": None},
        }
