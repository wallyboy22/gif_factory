    ...
    def add_legend(
        ...
        if is_discrete:
            ...
            cols, rows, tfont, _ = FrameProcessor._layout_discrete(
                width, entries, tfont, box_size, 10, margin, max_cols=4)
    ...
    def add_bottom_bar(
        ...
        if not show_legend:
            ...
            cols, rows, tfont, tfs = FrameProcessor._layout_discrete(
                width, entries, tfont, box_size, 8, margin, max_cols=2)
        ...
        if show_legend:
            if n == 0:
                ...
                for i in range(num_ticks):
                    ...
                    bbox = draw.textbbox((0, 0), lbl, font=tfont)
                    lw = bbox[2] - bbox[0]
                    draw.text((xi - lw // 2, bar_y_pos + box_size + 5), lbl, font=tfont, fill=(0, 0, 0))
