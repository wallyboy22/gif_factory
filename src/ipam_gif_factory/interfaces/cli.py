...
class CLI:
    ...
    def _execute_batch(self, batch_path: str, output_dir: Optional[str],
        ...
        def process_one(item):
            ...
            print(f"  ├── Dataset: {ds}  |  Território: {terr}")
        ...
        print("─" * 52)
        if errors:
            print(f"  ⚠️  {total - errors} concluído(s), {errors} erro(s)  |  {total_time:.1f}s")
        else:
            print(f"  ✅ {total} iten{'s' if total > 1 else ''} concluído{'s' if total > 1 else ''}  |  {total_time:.1f}s")
        print()
